...
　
   ...
   reader.onload = function() {
     ...
     var allKeyData=[];
      ...
      var itemArr = [];
      var onlyyear = onlyYear($('#graduate_prospects_input').val().toString());
      var graduate_prospects_date = dateWithoutDay($('#graduate_prospects_input').val().toString());
      ...
      var insert="";
          ...
          for (var i = 1; i < lineArr.length-1; i++) {
            ...
            insert+='<div class="publish_no">'+onlyyear+'-13-'+serialNo(String(i))+'</div>';
            insert+='<div class="text_title">指定保育士養成施設卒業見込証明書</div>';
            ...
            insert+='</div>'
            insert+='<div class="mid_text">　児童福祉法第十八条の六第一号の規定により指定された指定保育士養成施設において児童福祉法施行規則第六条の二第一項第三号の規定による修業教科目及び単位数を同号の規定により履修して卒業する見込の者であることを証明する。</div>';
            insert+='<div class="graduate_prospects_date">【'+graduate_prospects_date+'卒業見込】'+'</div>';
