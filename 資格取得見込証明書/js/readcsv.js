...
function handleFileSelect(evt) {
  if($('#certificate_prospects_input').val()==""){
...
　
   ...
   reader.onload = function() {
     ...
     var allKeyData=[];
      ...
      var itemArr = [];
      var onlyyear = onlyYear($('#certificate_prospects_input').val().toString());
      var certificate_prospects_date = dateWithoutDay($('#certificate_prospects_input').val().toString());
      // console.log(certificate_prospects_date);
      ...
      var insert="";
          ...
          for (var i = 1; i < lineArr.length-1; i++) {
            ...
            insert+='<div class="publish_no">'+onlyyear+'-16-'+serialNo(String(i))+'</div>';
            insert+='<div class="text_title">保育士資格取得見込証明書</div>';
            ...
            insert+='</div>'
            insert+='<div class="mid_text">　児童福祉法第十八条の六第一号の規定により指定された指定保育士養成施設において児童福祉法施行規則第六条の二第一項第三号の規定による修業教科目及び単位数を同号の規定により履修して保育士資格を取得する見込の者であることを証明する。</div>';
            insert+='<div class="certificate_prospects_date">【取得見込年月：'+certificate_prospects_date+'】'+'</div>';
